...
## Архитектура
    ...
    текстовый ответ (сразу и всегда)
       ...
       └─ [галочка] fal-ai/creatify/aurora (фолбэк: kling-video/ai-avatar/v2/standard) ─► видео
...
4. Приложение:
   ...
   ⚠️ Пока `FAL_MOCK=1`, голосовой вопрос **не распознаётся**: вместо этого
   подставляется заранее заданный текст с явным префиксом `[РЕЖИМ ЗАГЛУШКИ...]`,
   а аудио и видео — пустые файлы на 0 байт. Чтобы это нельзя было принять за
   настоящую работу, интерфейс в этом режиме показывает предупреждающий баннер,
   а статус после нажатия «Озвучить» прямо говорит, что файлы поддельные.
   Для настоящего пайплайна запускать надо с `FAL_MOCK=0`, как в команде выше.

## Скриншоты и видео-демо

Полный цикл в одном экране: вопрос задан **голосом** (в поле диалога видна
расшифровка Whisper), агент ответил по реальным данным 2GIS и честно сказал,
что скидок на эти заведения в Chocolife нет, ответ озвучен клонированным
голосом (плеер на 0:46) и превращён в видео с аватаром:

![Полный цикл: голос → ответ → аудио → видео с аватаром](assets/screenshot.png)

Тот же интерфейс с раскрытым логом вызовов инструментов — видно, что модель
сама обратилась к обоим MCP-серверам и сколько записей вернул каждый:

![Диалог с логом вызовов инструментов](assets/screenshot-chat.png)

Видео с аватаром на первом скриншоте — настоящий результат
`fal-ai/creatify/aurora` (832×1088, портретная ориентация исходного фото),
озвученный клонированным голосом владельца.

**Видео-демо:** `assets/demo.mp4` — работа приложения от голосового вопроса до
готового видео с аватаром.

## Тесты

    .venv/bin/pytest

111 тестов, ни один не ходит в сеть и не тратит деньги — сеть и fal
...
`cache/costs.jsonl` (копия журнала приложена к архиву как `assets/costs.jsonl`):

| Статья | Вызовов | Сумма |
|---|---|---|
| `fal-ai/whisper` (ASR) | 2 | $0.02 |
| `fal-ai/minimax/speech-02-hd` (TTS) | 4 | $0.08 |
| `fal-ai/creatify/aurora` (видео-аватар) | 2 | $2.00 |
...
- [x] ~~Скриншоты~~ — сделаны: `assets/screenshot.png` (полный цикл: голос →
      ответ → озвучка → видео) и `assets/screenshot-chat.png` (живой прогон с
      реальными данными 2GIS и Chocolife и раскрытым логом инструментов).
- [x] ~~Записать `assets/demo.mp4`~~ — записано (75 секунд: голосовой вопрос,
      работа агента, озвучка и видео с аватаром).
- [ ] Собрать архив: `./tools/make_zip.sh Gizzat_Tazabekov` и проверить его
      содержимое перед сдачей.

## Что лежит в архиве, но не в git

Задание требует приложить к архиву исходники аватара, поэтому в ZIP попадают
`voice/my_voice_sample.m4a` и `avatar/my_photo.jpg` — без них проверяющий не
сможет запустить голосовую часть на своём ключе. В публичный репозиторий они
не коммитятся (см. `.gitignore`): это личные голос и фотография.

Сгенерированные медиа (`output/`) в архив не входят — вместо них приложены
скриншоты живых прогонов и видео-демо `assets/demo.mp4`.
